...
public interface UserProfileRepository extends JpaRepository<UserProfile, Long> {
    ...
    @Query(value = "SELECT * FROM user_profile WHERE LOWER(created_by) = LOWER(:managerUsername) AND active = true", nativeQuery = true)
    List<UserProfile> findActiveByManagerUsername(@Param("managerUsername") String managerUsername);

    @Query(value = "SELECT * FROM user_profile WHERE LOWER(role) = LOWER(:role) AND active = true", nativeQuery = true)
    List<UserProfile> findActiveByRole(@Param("role") String role);

    @Query(value = "SELECT * FROM user_profile WHERE active = true AND id != :excludeId", nativeQuery = true)
